...
function updateHintButton() {
  ...
  if (hintButton) {
    const maxHints = window.gameState.maxHints || 3;
    if (window.gameState.currentHintLevel >= maxHints) {
      hintButton.disabled = true;
      hintButton.textContent = 'No more hints';
    } else {
      hintButton.disabled = false;
      hintButton.textContent = `Get a hint (${window.gameState.currentHintLevel}/${maxHints} used)`;
    }
  }
}
